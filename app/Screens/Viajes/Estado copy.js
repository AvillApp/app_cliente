  ...
  const getIdpedido = async () => {
    ...
    if (id_pedido) {
      ...
      const response2 = await fetch(
        `${api}activiorders/?pedido=${id_pedido}&format=json`
      );
      ...
      const infoUser = await fetch(`${api}orders/${id_user}/?format=json`);
      ...
      if (estado_ped == 5) {
        ...
        //setPrecio(estado_pedido.precio); // Conductor disponible
        setTxt(
          "¿Aceptar pedido? \n \n Valor:" +
            precio +
            "\n Placa: " +
